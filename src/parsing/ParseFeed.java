...
public class ParseFeed {
	 ...
	 * @return A HashMap of country->average age of death
	 */
	public static HashMap<String, Float> loadGDPGrowthFromCSV(PApplet p, String fileName) {
		// HashMap key: country ID and  data: lifeExp at birth
		HashMap<String, Float> GDPgrowthMap = new HashMap<String, Float>();
		...
		for (String row : rows) {
			...
			for(int i = columns.length - 1; i > 3; i--) {
				...
				// check if value exists for year
				if(!columns[i].equals("..")) {
					GDPgrowthMap.put(columns[3], Float.parseFloat(columns[i]));
	...
	public static HashMap<String, String> loadCountryName(PApplet p, String fileName) {
		// HashMap key: country ID and  data: lifeExp at birth
		...
		for (String row : rows) {
			 ...
			 */
			//
			CountryName.put(columns[3], columns[2]);		
		}	
		
		return CountryName;
	}

	public static HashMap<String, Float> loadCountryEmpty(PApplet p, String fileName) {
		// HashMap key: country ID and  data: lifeExp at birth
		HashMap<String, Float> CountryName = new HashMap<String, Float>();
		...
		for (String row : rows) {
			 ...
			 */
			//
			CountryName.put(columns[3], def);		
		}	
		
		return CountryName;
	}	
	
	public static HashMap<String, Float> loadGDPCapitaFromCSV(PApplet p, String fileName) {
		// HashMap key: country ID and  data: lifeExp at birth
		HashMap<String, Float> GDPCapitaMap = new HashMap<String, Float>();
		...
		for (String row : rows) {
			...
			for(int i = columns.length - 1; i > 3; i--) {
				...
				// check if value exists for year
				if(!columns[i].equals("..")) {
					GDPCapitaMap.put(columns[3], Float.parseFloat(columns[i]));
